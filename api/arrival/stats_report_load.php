...
$query1 ="select * from  move where arrival = '1'  AND  vessel_id = '$vessel_id' AND seer != 'بدون سيور شحن' GROUP BY seer";
$result1 = mysqli_query($con, $query1);
$i=0;
$output1 .= '
        ...
        <thead>
            <tr>
             <th style="background-color:'.$colors[$i].'; color:white; border: 1px solid black;padding: 1px;">رقم السير</th>
               
                  <th style="background-color:'.$colors[$i].'; color:white; border: 1px solid black;padding: 1px;">عدد النقلات </th>
                  
 <th style="background-color:'.$colors[$i].'; color:white; border: 1px solid black;padding: 1px;">الكمية بالطن </th>
            </tr>
...
if ($result1->num_rows > 0) {
    while ($row1 = mysqli_fetch_assoc($result1)) {
        ...
        $query11 ="select SUM(qnt) as qnts ,seer,COUNT(id) as count from move where seer = '".$row1["seer"]."' AND vessel_id = '$vessel_id'  ";
        $result11= mysqli_query($con, $query11);
        ...
        $output1 .= '<tr>  
             <th color:black; border: 1px solid black;padding: 1px;font-size:12px;">   <span style="color:black;">' . $row11["seer"] . '</span> </th>
               <th color:black; border: 1px solid black;padding: 1px;font-size:12px;">   <span style="color:black;">'.$row11["count"]. ' </span> </th>
                 <th color:black; border: 1px solid black;padding: 1px;font-size:12px;">   <span style="color:black;">' . number_format((float)$qnts1, 3, '.', '')  . ' </span> </th>
...
if ($result2->num_rows > 0) {
    while ($row2 = mysqli_fetch_assoc($result2)) {
        $name2 = $row2["store_no"];
        $query22 = "select SUM(qnt) as qnts , store_no ,COUNT(id) as count from move where store_no = '$name2' AND  vessel_id = '$vessel_id' ";
        ...
        $output2 .= '<tr>  
             <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">'.$row22["store_no"].'</span> </th>
              <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">'.$row22["count"]. ' </span> </th>
                          <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">' . number_format((float)$qnts2, 3, '.', '')  . ' </span> </th>
...
if ($result8->num_rows > 0) {
    while ($row8 = mysqli_fetch_assoc($result8)) {
        $name8 = $row8["type"];
        $query88 = "select SUM(qnt) as qnts , type ,COUNT(id) as count from move where type = '$name8' AND  vessel_id = '$vessel_id' ";
        ...
        $output8 .= '<tr>  
             <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">'.$row88["type"].'</span> </th>
              <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">'.$row88["count"]. ' </span> </th>
                          <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">' . number_format((float)$qnts8, 3, '.', '')  . ' </span> </th>
...
$query3 ="select * from  move where arrival = '1'  AND  vessel_id = '$vessel_id' GROUP BY room_no  order by room_no asc";
...
if ($result3->num_rows > 0) {
    ...
    while ($row3 = mysqli_fetch_assoc($result3)) {
        ...
        $query33 ="select SUM(qnt) as qnts ,SUM(jumbo) as jumbo ,room_no ,COUNT(id) as count from move where room_no = '".$row3["room_no"]."'  AND  vessel_id = '$vessel_id'  ";
        $result33= mysqli_query($con, $query33);
...
$output3 .='</tbody></table>';
/*
$query4 ="select * from  move  where  vessel_id = '$vessel_id'  AND arrival = '1'  AND hla1 != 'بدون حِلل' GROUP BY hla1";
$result4 = mysqli_query($con, $query4);
$i=3;
$output4 .= '
        	<table  id="customers" style= "margin-bottom:5px;">
...
if ($result4->num_rows > 0) {
    while ($row4 = mysqli_fetch_assoc($result4)) {
        $query44 ="select SUM(qnt) as qnts ,hla1,COUNT(id) as count from move where hla1 = '".$row4["hla1"]."' AND  vessel_id = '$vessel_id'";
        $result44= mysqli_query($con, $query44);
...
$query4 ="select * from  move  where  vessel_id = '$vessel_id'  AND arrival = '1'  AND hla2 != 'بدون حِلل' GROUP BY hla2";
$result4 = mysqli_query($con, $query4);
$i=3;

if ($result4->num_rows > 0) {
    while ($row4 = mysqli_fetch_assoc($result4)) {
        $query44 ="select SUM(qnt) as qnts ,hla2,COUNT(id) as count from move where hla2 = '".$row4["hla2"]."' AND  vessel_id = '$vessel_id'";
        $result44= mysqli_query($con, $query44);
        if(mysqli_num_rows($result44) > 0) {
            $row44= mysqli_fetch_assoc($result44);
            $qnts4 = $row44['qnts'];
...
$query5 ="select * from  move where vessel_id = '$vessel_id' AND arrival = '1' AND kbsh !='بدون كباشات أوناش'  GROUP BY kbsh    ";
...
if ($result5->num_rows > 0) {
    while ($row5 = mysqli_fetch_assoc($result5)) {
        $query55 ="select SUM(qnt) as qnts , kbsh,COUNT(id) as count  from move where kbsh = '".$row5["kbsh"]."' AND vessel_id = '$vessel_id' ";
 ...
 <tr>  
             <th color:black; border: 1px solid black;padding: 1px;font-size:12px;">   <span style="color:black;">'.$row55["kbsh"].'</span> </th>
              ...
              <th color:black; border: 1px solid black;padding: 1px;font-size:12px;">   <span style="color:black;">' . number_format((float)$qnts5, 3, '.', '')  . ' </span> </th>
...
$query6="select * from  move where vessel_id = '$vessel_id' AND  arrival = '1' AND crane != 'بدون أوناش' GROUP BY crane ";
...
if ($result6->num_rows > 0) {
    while ($row6 = mysqli_fetch_assoc($result6)) {
        $query66 ="select SUM(qnt) as qnts ,crane,COUNT(id) as count  from move where crane = '".$row6["crane"]."' AND vessel_id = '$vessel_id' ";
 ...
 <tr>  
             ...
             <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">' . number_format((float)$qnts6, 3, '.', '')  . ' </span> </th>
            </tr>';
    }
}
$output6 .='</tbody></table>';
...
$output77 ='
        	<div class ="col-lg-8">
        <table  id="customers"  style style= "margin-bottom:5px;">
        <thead>
            <tr>
   ...
   if ($result9->num_rows > 0) {
    while ($row9 = mysqli_fetch_assoc($result9)) {
       ...
       $query99 ="select * from  cars where car_owner = '$car_owner'  AND  vessel_id = '$vessel_id' ";
        ...
        $query099 ="select SUM(qnt) as qnts ,COUNT(id) as count  from move where sn = '$cars[$s]'  AND vessel_id = '$vessel_id' ";
...
$query9 ="select * from  move where vessel_id = '$vessel_id' AND arrival = '1'  GROUP BY room_no    ";
$result9= mysqli_query($con, $query9);
$i=4;
$room_no =[];
$output9 ='
        ...
        <thead>
            <tr>
              <th style="background-color:'.$colors[$i].'; color:white; border: 1px solid black;">رقم السير</th>';
            if ($result9->num_rows > 0) {
              while ($row9 = mysqli_fetch_assoc($result9)) {
        ...
        $seer =[];
    $query99 ="select * from  move where arrival = '1'  AND  vessel_id = '$vessel_id' AND seer != 'بدون سيور شحن' GROUP BY seer";
        $result99= mysqli_query($con, $query99);
        if ($result99->num_rows > 0) {
          while ($row99 = mysqli_fetch_assoc($result99)) {
               array_push($seer,$row99["seer"]);
         }
         }
         for($s=0;$s<count($seer);$s++){
        $output9 .= '
                <tr>  
             <th color:black; border: 1px solid black;font-size:12px;">   <span style="color:black;">'.$seer[$s].'</span> </th>
             ';
              for($f=0;$f<count($room_no);$f++){
                 $query099 ="select SUM(qnt) as qnts ,COUNT(id) as count  from move where seer = '$seer[$s]' AND room_no ='$room_no[$f]'  AND vessel_id = '$vessel_id' ";
        $result099= mysqli_query($con, $query099);
        if(mysqli_num_rows($result099) > 0) {
            $row099= mysqli_fetch_assoc($result099);
            $qnts9 = $row099['qnts'];
...
$output19 ='
        ...
        $type =[];
    $query99 ="select * from  move where arrival = '1'  AND  vessel_id = '$vessel_id'  GROUP BY type";
        $result99= mysqli_query($con, $query99);
        if ($result99->num_rows > 0) {
          while ($row99 = mysqli_fetch_assoc($result99)) {
        ...
        $output19 .= '
             ...
             ';
              for($f=0;$f<count($room_no);$f++){
                 $query099 ="select SUM(qnt) as qnts ,COUNT(id) as count  from move where type = '$type[$s]' AND room_no ='$room_no[$f]'  AND vessel_id = '$vessel_id' ";
        $result099= mysqli_query($con, $query099);
        if(mysqli_num_rows($result099) > 0) {
            $row099= mysqli_fetch_assoc($result099);
            $qnts9 = $row099['qnts'];
...
$query10 ="select * from  move where vessel_id = '$vessel_id' AND arrival = '1'  GROUP BY room_no    ";
...
$output10 ='
        ...
        <thead>
            <tr>
               <th style="background-color:'.$colors[$i].';color:white; border: 1px solid black;padding: 1px;font-size:12px;">
        ...
        $crane =[];
    $query1010 ="select * from  move where arrival = '1'  AND  vessel_id = '$vessel_id' AND crane != 'بدون أوناش' GROUP BY crane";
        ...
        $output10 .= '
             ...
             ';
              for($f=0;$f<count($room_no);$f++){
                 $query01010 ="select SUM(qnt) as qnts ,COUNT(id) as count  from move where crane = '$crane[$s]' AND room_no ='$room_no[$f]'  AND vessel_id = '$vessel_id' ";
...
$output17 .='</tbody></table>';
  ...
  $query11 ="select * from  move where  vessel_id = '$vessel_id' AND room_no != ''";
  $result11= mysqli_query($con, $query11); $room_no=1; if ($result11->num_rows == 0) $room_no=0;
    $query11 ="select * from  move where  vessel_id = '$vessel_id' AND seer != 'بدون سيور شحن'";
  $result11= mysqli_query($con, $query11); $seer=1; if ($result11->num_rows == 0) $seer=0;      
    $query11 ="select * from  move where  vessel_id = '$vessel_id' AND hla1 != 'بدون حِلل'";
  $result11= mysqli_query($con, $query11); $hla1=1; if ($result11->num_rows == 0) $hla1=0;  
   $query11 ="select * from  move where  vessel_id = '$vessel_id' AND kbsh != 'بدون كباشات أوناش'";
  $result11= mysqli_query($con, $query11); $kbsh=1; if ($result11->num_rows == 0) $kbsh=0;  
   $query11 ="select * from  move where  vessel_id = '$vessel_id' AND crane != 'بدون أوناش'";
...
//seer
if($seer =='1') echo $output1;
//seer-room_no
if($seer =='1' AND $room_no=='1')echo $output9;
